...
def build_theme_css_vars(theme):
    ...
    return (
        ...
        f"--img-radius:8px;"
        f"--img-border-width:1px;"
        f"--img-shadow-blur:0px;"
    )
...
def render_paragraph(text, theme):
    ...
    return (f'<p style="margin:0 0 17px;font-size:15.5px;line-height:1.95;letter-spacing:0.2px;'
...
def render_image(name, caption, theme, search_dirs):
    ...
    cap_html = (f'<p style="margin:10px 0 30px;font-size:12px;color:var(--c-mute);'
                f'line-height:1.7;text-align:center;letter-spacing:0.5px;">图注 / {cap}</p>') if cap else ''
    is_scrollbox = any(k in name for k in ('全图', '长图', 'scrollbox', 'longshot'))
    if is_scrollbox:
        return (f'<section style="margin:30px auto 0;text-align:center;background:#fafafa;padding:8px;">'
                ...
                f'box-shadow:0 4px var(--img-shadow-blur) rgba(0,0,0,0.04);background:#fff;">'
    ...
    return (f'<section style="margin:30px auto 0;text-align:center;background:#fafafa;padding:8px;">'
            ...
            f'border-radius:var(--img-radius);border:var(--img-border-width) solid var(--c-img-border);'
            f'box-shadow:0 4px var(--img-shadow-blur) rgba(0,0,0,0.04);">'
            f'</section>{cap_html}')
...
def render_quote(text, theme):
    ...
    if re.match(r'^https?://\S+$', stripped):
        return (f'<p style="margin:22px 0;font-size:15px;line-height:1.85;word-break:break-all;">'
                ...
                f'{stripped}</a></p>')
    return (f'<section style="margin:26px 0;padding:2px 0 2px 16px;'
            f'border-left:3px solid var(--c-main);background:transparent;">'
            f'<p style="margin:0;font-size:15px;line-height:1.9;color:var(--c-text);">'
            f'{inline(text, theme)}</p></section>')
...
def render_h2(text, theme):
    return (f'<section style="margin:46px 0 22px;padding-top:18px;border-top:1px solid #ececec;">'
            f'<span style="display:inline-block;font-size:12px;color:var(--c-main);'
            f'letter-spacing:2px;font-weight:700;margin-bottom:8px;">SECTION</span>'
            f'<p style="margin:0;font-size:21px;font-weight:800;color:var(--c-text-strong);'
            f'line-height:1.45;letter-spacing:0.2px;">{inline(text, theme)}</p>'
            f'</section>')


def render_h3(text, theme):
    return (f'<section style="margin:32px 0 13px;">'
            f'<p style="margin:0;font-size:var(--h3-size);font-weight:800;color:var(--c-text-strong);'
            f'line-height:1.45;letter-spacing:0.2px;">'
            f'<span style="color:var(--c-main);font-weight:900;margin-right:7px;">/</span>'
            f'{inline(text, theme)}</p></section>')


def render_ul(items, theme):
    items_html = ''.join(
        f'<section style="margin:7px 0;font-size:15px;line-height:1.85;letter-spacing:0.2px;'
        f'color:var(--c-text);">'
        f'<span style="display:inline-block;width:5px;height:5px;background:var(--c-main);'
        f'border-radius:50%;margin:0 10px 2px 1px;"></span>'
        f'<span>{inline(it, theme)}</span></section>'
        for it in items
    )
    return f'<section style="margin:18px 0 20px;padding-left:2px;">{items_html}</section>'


def render_ol(items, theme):
    items_html = ''.join(
        f'<section style="margin:9px 0;font-size:15px;line-height:1.85;letter-spacing:0.2px;'
        f'color:var(--c-text);">'
        f'<span style="display:inline-block;min-width:24px;color:var(--c-main);font-weight:800;'
        f'font-size:13px;letter-spacing:0.5px;">{idx + 1}.</span>'
        f'<span>{inline(it, theme)}</span></section>'
...
def render_step_card(num, title, theme, label_text=None):
    ...
    label_html = (f'<span style="flex-shrink:0;font-size:11px;color:var(--c-faint);'
                  ...
                  if label else '')
    return (f'<section style="margin:52px 0 24px;">'
            f'<section style="display:flex;align-items:stretch;gap:16px;padding:18px 0 16px;'
            f'border-top:1px solid #ececec;border-bottom:1px solid #ececec;">'
            f'<section style="flex-shrink:0;width:70px;min-height:70px;'
            f'display:flex;align-items:center;justify-content:center;">'
            f'<span style="font-size:42px;font-weight:900;color:var(--c-main);line-height:1;'
            f'letter-spacing:0.5px;">{num:02d}</span></section>'
            f'<section style="flex:1;display:flex;flex-direction:column;justify-content:center;">'
            f'<section style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">'
            f'<span style="font-size:11px;color:var(--c-main);letter-spacing:1.5px;font-weight:800;">'
            f'<span data-stamp>{stamp}</span></span>{label_html}</section>'
            f'<p style="margin:0;font-size:22px;font-weight:800;color:var(--c-text-strong);'
            f'line-height:1.4;letter-spacing:0.1px;">{inline(title, theme)}</p></section>'
            f'</section></section>')
...
def render_article_body(md_text, theme, img_dir=None, plain=False):
    ...
    if plain:
        ...
        head = (
            f'<section style="margin:0 0 34px;padding:0 0 26px;border-bottom:1px solid #ededed;">'
            f'<section style="display:flex;align-items:center;justify-content:space-between;gap:12px;'
            f'margin-bottom:24px;">'
            f'<span style="font-size:12px;color:var(--c-main);font-weight:800;letter-spacing:1.5px;">'
            f'<span data-brand>{b["name"]}</span></span>'
            f'<span style="font-size:11px;color:var(--c-mute);letter-spacing:1px;">'
            f'<span data-issue-prefix>{b["issue_prefix"]}</span><span data-vol>{b["issue_number"]:03d}</span> · {date}</span>'
            f'</section>'
            f'<p style="margin:0 0 12px;font-size:26px;font-weight:850;color:var(--c-text-strong);'
            f'line-height:1.35;letter-spacing:0.1px;">{title}</p>'
            f'<p style="margin:0;font-size:16px;font-weight:500;color:var(--c-text);'
            f'line-height:1.75;">{subtitle}</p>'
            f'<section style="margin-top:18px;display:flex;align-items:center;gap:10px;">'
            f'<section style="width:32px;height:2px;background:var(--c-main);"></section>'
            f'<span style="font-size:11px;color:var(--c-mute);letter-spacing:1px;">{feature}</span>'
            f'</section>'
            f'</section>'
        )
        tail = (
            f'<section style="margin:54px 0 0;padding-top:20px;border-top:1px solid #ededed;'
            f'display:flex;align-items:center;justify-content:space-between;gap:12px;">'
            f'<span style="font-size:11px;color:var(--c-text-strong);letter-spacing:1px;font-weight:700;">'
            f'<span data-brand>{b["name"]}</span> {b["magazine_suffix"]}</span>'
            f'<span style="font-size:10px;color:var(--c-mute);letter-spacing:1px;">感谢阅读</span>'
            f'</section>'
        )
